...
class RoundPlotter:
    ...
    def update_predator_trajectories(self, pred_datas: List[List[Tuple[float, float]]], ax: plt.Axes, t: int, z: float, colors: Iterable[str]) -> None:
    ...
    def update_trajectories(self, agent_datas: List[List[Tuple[float, float]]], pred_datas: List[List[Tuple[float, float]]],
        ...
        # predator data
        self.update_predator_trajectories(pred_datas=pred_datas, ax=ax, t=t, z=z, colors=colors)
    ...
    def update_predator_lines(self, data_list: List[NDArray[float]], ax: plt.Axes, lines: List[plt.Line2D],
                              t: int, t_start: int, time_window_dur: float, colors: Iterable[str],
                              linestyle: str = '-', alpha: float =1.) -> None:
    ...
    def plot_metrics(self, time_window_dur: float = 2000., smoothing_args: Dict = None,
        ...
        def update(t: int, args_dict: Dict) -> Dict:
            ...
            self.update_trajectories(agent_datas=self.round.agent_datas, pred_datas=self.round.pred_datas,
                                     ax=args_dict['ax'], t=t, z=z, colors=colors, com_only=com_only, agent_com=self.round.agent_com)
            ...
            self.update_predator_lines(pred_datas_vel, ax=args_dict['ax_vel'], lines=[args_dict[f'vel_{pid}'] for pid in range(self.round.n_preds)],
                                       t=t, t_start=t_start, time_window_dur=time_window_dur, colors=colors )
            ...
            self.update_predator_lines(pred_datas_acc, ax=args_dict['ax_acc'],
                                       ...
                                       t=t, t_start=t_start, time_window_dur=time_window_dur, colors=colors, linestyle='--', alpha=0.6)
        ...
        for pid in range(self.round.n_preds):
            args_dict[f'vel_{pid}'], = ax_vel.plot([], [], color=colors[pid][:-1])
            args_dict[f'acc_{pid}'], = ax_acc.plot([], [], color=colors[pid][:-1])
            args_dict[f'acc_smoothed_{pid}'], = ax_acc.plot([], [], color=colors[pid][:-1])
            args_dict[f'com_{pid}'], = ax_com.plot([], [], color=colors[pid][:-1])
            args_dict[f'bor_{pid}'], = ax_bor.plot([], [], color=colors[pid][:-1])
    ...
    def plot_predator_acc_smoothings(self, time_window_dur: float = 2000., window_size: int = 40,
        ...
        def update(t: int, args_dict: Dict) -> Dict:
            ...
            self.update_predator_lines([np.concatenate([np.array([0]), dts])], ax=args_dict['ax_dts'],
                                      lines=[args_dict['dts']],
                                       t=t, t_start=t_start, time_window_dur=time_window_dur, colors=['Blacks'])
        ...
        for pid in range(self.round.n_preds):
            args_dict[f'acc_{pid}'], = ax_acc.plot([], [], color=colors[pid][:-1])
            args_dict[f'acc_w_{pid}'], = ax_acc_w.plot([], [], color=colors[pid][:-1])
            args_dict[f'acc_w_gaus_{pid}'], = ax_acc_w_gaus.plot([], [], color=colors[pid][:-1])
            args_dict[f'acc_bw_{pid}'], = ax_acc_bw.plot([], [], color=colors[pid][:-1])

            args_dict[f'acc_w_raw_{pid}'], = ax_acc_w.plot([], [], color=colors[pid][:-1])
            args_dict[f'acc_w_gaus_raw_{pid}'], = ax_acc_w_gaus.plot([], [], color=colors[pid][:-1])
            args_dict[f'acc_bw_raw_{pid}'], = ax_acc_bw.plot([], [], color=colors[pid][:-1])
    ...
    def plot_bouts(self, time_window_dur: float = 2000., com_only: bool = True):
        def update(t: int, args_dict: Dict) -> Dict:
            t_start = max([0, find_nearest(self.round.timestamps, self.round.timestamps[t] - 2 * time_window_dur / 3)])
            z = self.round.timestamps[t]
            ...
            self.update_trajectories(agent_datas=self.round.agent_datas_arr_bouts, pred_datas=self.round.pred_datas_arr_bouts,
                                     ax=args_dict['ax'], t=t, z=z, colors=colors, com_only=com_only, agent_com=self.round.agent_com_bouts)
